...
describe('iChoveu Project test', () => {
  ...
  describe('3. Liste as cidades retornadas pela API', () => {
    it('Verifica se a lista de cidades é exibida corretamente', () => {
      ...
      cy.get('@fetch').then((fetch) => {
        const fetchPromises = RioSearchData.map((city) => {
          return fetch.withArgs(`http://api.weatherapi.com/v1/current.json?key=YOUR_API_KEY&q=${city.url}`)
            .returns({ json: () => currentData[city.url] });
        });

        Promise.all(fetchPromises).then(() => {
          const citiesList = cy.get('#cities');

          RioSearchData.forEach((city) => {
            const expectedData = currentData[city.url];
            const cityElement = createCityElement({
              name: city.name,
              country: city.country,
              temp: expectedData.current.temp_c,
              condition: expectedData.current.condition.text,
              icon: expectedData.current.condition.icon,
              url: city.url,
            });

            citiesList.invoke('append', cityElement);
          });
        });
      });
    });
